...
**These bytes are generated by the production argument encoders in `src/instructions/`, not hand-written.** They are NOT what `@1inch/swap-vm-sdk` emits — that package targets a different swap-vm version, with an entirely different opcode table (its `onlyTakerTokenBalanceNonZero` is `0x0e`, not `0x23`). See `../DEMO.md` §6.
...
| | |
|---|---|
| Semantics | written (`../swapvm.md`) |
| Conformance | passing, both engines |
| Theorem | **proved** (`../proofs/gate-spec.k`, `#Top`) |
